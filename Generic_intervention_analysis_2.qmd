...
f_name = here("Analysis_Data", paste0(intervention, "_analysis.csv"))
...
```{r}
plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_recruitment.png"))
print(plot_name)
...
```{r}
plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_mITT_timeline.png"))
...
```{r}
plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_base_vl_flutype.png"))

png(plot_name, width = 8, height = 4, units = "in", res = 600)
base_vl_plot
dev.off()
```
...
```{r}
kable_path <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_Baseline_characteristics.png")
)
save_kable(tab.ff, file = kable_path)
```
...
```{r}
Baseline_data %>%
  summarise(onset_q1 = quantile(symptomDay,0.25),
            onset_q2 = quantile(symptomDay,0.5),
            onset_q3 = quantile(symptomDay,0.75)
  )
```
...
fname <- here("Plots", intervention, paste0("Baseline_vl_", "mITT_", mITT_threshold, ".png"))

png(fname, width = 8, height = 4, units = "in", res = 600)
G_baseline_vl
dev.off()
```

```{r}
t.test(Baseline.viral.load ~ fluType, data = Baseline_data)
```

## Viral load dynamics

```{r, warning=F}
box_vl_flutype <- plot_vl_box(adastra_dat_analysis, trt_colors, fluType = T)
box_vl_flutype
```

```{r}
plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_box_vl_flutype.png"))
...
```{r box_vl, warning=F}
box_vl <- plot_vl_box(adastra_dat_analysis, trt_colors, fluType = F)
box_vl
```

```{r}
plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_box_vl.png"))
...
covs_base = c('symptomDay','fluType', 'Study_time', 'Age_scaled', 'vaccine')
...
model_settings = expand.grid(mod = all_mods[2], # LinearRNA stan
                             prior = 1,
                             cov_matrices = 1,
                             trt_formulas = seq_len(length(stan_inputs)))
                             #trt_formulas = c(1,2)) #c(1,2))
...
model_setup_f_name = here("Rout", paste0('model_run_setup_',intervention, "_mITT_", mITT_threshold, '_vs_', ifelse(ref_arm == "No Study Drug", "NSD", ref_arm), '.RData'))
save(model_settings, 
     adastra_dat_analysis,
     stan_inputs, 
     all_priors,
...
```{r}
main_mod = 1# which(model_settings$prior==2&
#  model_settings$cov_matrices==1&
# model_settings$mod==all_mods[2] &
#model_settings$trt_formulas == 1)
...
```{r}
G_hl <- plot_hl(Half_life, trt_colors, trt_order=c("Baloxavir", "Favipiravir", "Oseltamivir", "No Study Drug"))
G_hl
```

```{r}
G_hl_flutype <- plot_hl_flutype(Half_life, trt_colors, trt_order=c("Baloxavir", "Favipiravir", "Oseltamivir", "No Study Drug"))
G_hl_flutype
```

```{r}
plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_hl_flutype.png"))

png(plot_name, width = 8, height = 8, units = "in", res = 800)
G_hl_flutype
dev.off()
```
...
```{r}
G2 <- plot_trt_effs(effect_ests = effect_ests, model_cols = model_cols, trt_order=c("Baloxavir", "Favipiravir", "Oseltamivir"))
G2
```

```{r}
plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_t12_trt.png"))

png(plot_name, width = 12, height = 5.5, units = "in", res = 800)
plot_grid(G_hl, G2, ncol = 2, align = "hv", axis = "b")
dev.off()
```

```{r}
G3 <- plot_trt_effs_2(effect_ests = effect_ests, model_cols = model_cols, -50, 40)
G3
```

```{r}
plot_name_pos <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_esimated_effect_vs_", ref_arm, ".png"))
...
png(plot_name, width = 6, height = 6, units = "in", res = 600)
print(G2 + 
        ggtitle(paste0("Estimated treatment effect relative to\n", tolower(ref_arm) , " arm")) 
)
dev.off()
...
model_selects <- which(model_settings$trt_formulas == 1)
...
for(i in 1:length(ind_plot_all)){
  fname <- paste0(prefix_dropbox, "/Interim Analysis Reports/Plots_", interim_version, "/", intervention,  "_mITT_", mITT_threshold, "_individual_", i, ".pdf")
