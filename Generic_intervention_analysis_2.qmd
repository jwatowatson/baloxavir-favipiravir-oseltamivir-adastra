...
```{r, warning=F}
box_vl_flutype <- plot_vl_box(adastra_dat_analysis, trt_colors, fluType = T, trt_order = trt_order)
box_vl_flutype
```
...
png(plot_name, width = 9, height = 8, units = "in", res = 800)
G_hl_flutype
dev.off()
```
...
```{r}
G2 <- plot_trt_effs(effect_ests = effect_ests, model_cols = model_cols, trt_order)
G2
```
