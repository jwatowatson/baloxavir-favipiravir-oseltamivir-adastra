...
```{r}
plot_name <- here("Plots", intervention, paste0("timeline.png"))
...
```{r}
plot_name <- here("Plots", intervention, paste0("timeline.pdf"))
pdf(plot_name, width = 8, height = 6)
print(timeline)
dev.off()
```
...
```{r}
plot_name <- here("Plots", intervention, paste0("mITT_per_symptomOnset.png"))
png(plot_name, width = 8, height = 6, units = "in", res = 600)
print(mITT_plot)
dev.off()
```
...
```{r}
plot_name <- here("Plots", intervention, paste0("mITT_per_symptomOnset.pdf"))
pdf(plot_name, width = 8, height = 6)
print(mITT_plot)
dev.off()
```
...
```{r}
mITT_count <- adastra_dat %>%
  mutate(
    symptomOnset = ceiling(symptomDay)
  ) %>%
  group_by(Trt, symptomOnset, mITT) %>%
  summarise(
    n_ID = n_distinct(ID),
    .groups = "drop"
  ) %>%
...
adastra_dat = adastra_dat %>% group_by(ID, Timepoint_ID) %>%
  ...
  mutate(Site = as.factor(Site),
         ...
         Sex = relevel(Sex, ref = "Female"), # Make female the reference
...
```{r}
plot_name <- here("Plots", intervention, paste0("baseline_vl_fluType.png"))
png(plot_name, width = 8, height = 6, units = "in", res = 600)
print(G_baseline_vl)
dev.off()
```
...
```{r}
plot_name <- here("Plots", intervention, paste0("baseline_vl_fluType.pdf"))
pdf(plot_name, width = 8, height = 6)
print(G_baseline_vl)
dev.off()
```
...
```{r}
# Baseline viral load statistics, stratified by flu type
Baseline_data %>%
  group_by(fluType) %>%
  summarise(
    n = sum(!is.na(Baseline.viral.load)),
    
    mean_vl   = round(mean(Baseline.viral.load, na.rm = TRUE), 2),
    sd_vl     = round(sd(Baseline.viral.load, na.rm = TRUE), 2),
    
    q1_vl     = round(quantile(Baseline.viral.load, 0.25, na.rm = TRUE), 2),
    median_vl = round(quantile(Baseline.viral.load, 0.5,  na.rm = TRUE), 2),
    q3_vl     = round(quantile(Baseline.viral.load, 0.75, na.rm = TRUE), 2),
    
    iqr_vl    = round(IQR(Baseline.viral.load, na.rm = TRUE), 2),
    
    .groups = "drop"
  )
```

## Left-Right Swab Difference

```{r}
# Investigate left-right diff

# Some Location names are wrong need fix
adastra_dat_swab_analysis <- adastra_dat_analysis %>%
  mutate(
    Location = str_trim(Location),
    # Fix TSL into TLS
    Location = str_replace(Location, "^TSL", "TLS"),
    # Remove hyphen variants (TSL-1 into TLS1, RTS-1 into RTS1)
    Location = str_replace(Location, "-", ""),
    # Remove stray "-" rows
    Location = ifelse(Location == "", NA, Location)
  )

adastra_dat_swab_analysis <- adastra_dat_swab_analysis %>%
  mutate(
    replicate = case_when(
      Location %in% c("TLS1", "RTS1") ~ 1,
      Location %in% c("TLS2", "RTS2") ~ 2,
      Location %in% c("TLS",  "RTS")  ~ 1,  # follow-up only one swab pair
      TRUE ~ NA_real_
    ),
    swab_side = case_when(
      grepl("^TLS", Location) ~ "TLS",
      grepl("^RTS", Location) ~ "RTS",
      TRUE ~ NA_character_
    )
  )
```
...
```{r}
# Only take swabs that have been labeled
adastra_dat_pairs <- adastra_dat_swab_analysis %>%
  filter(swab_side %in% c("TLS", "RTS")) %>%
  select(ID, Timepoint_ID, replicate, swab_side, log10_viral_load)

# Want only exactly one TLS and RTS in each ID, Timepoint_ID, replicate
adastra_dat_pairs <- adastra_dat_pairs %>%
  group_by(ID, Timepoint_ID, replicate) %>%
  filter(n_distinct(swab_side) == 2, sum(swab_side == "TLS") == 1, sum(swab_side == "RTS") == 1) %>%
  ungroup()

adastra_dat_diff <- adastra_dat_pairs %>%
  pivot_wider(names_from = swab_side, values_from = log10_viral_load) %>% mutate(diff_TLS_minus_RTS = TLS - RTS, 
          abs_diff = abs(diff_TLS_minus_RTS),
          mean_vl = (TLS + RTS) / 2)
```

```{r}
box_lr_swab <- plot_swab_lr_boxplot(adastra_dat_pairs)
box_lr_swab
```

```{r}
plot_name <- here("Plots", intervention, paste0("box_lr_swab.png"))
png(plot_name, width = 8, height = 6, units = "in", res = 600)
print(box_lr_swab)
dev.off()
```
...
```{r}
ggplot(adastra_dat_diff, aes(x = diff_TLS_minus_RTS)) +
  geom_histogram(bins = 30, color = "black", fill = "skyblue") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  theme_bw(base_size = 14) +
  labs(
    x = "Right - Left (log10 viral load)",
    y = "Count",
    title = "Distribution of paired swab differences"
  )
```

```{r}
adastra_dat_pairs$swab_side <- factor(adastra_dat_pairs$swab_side, , levels = c("TLS", "RTS"))

fit_side <- lmer(
  log10_viral_load ~ swab_side + Timepoint_ID + (1 | ID), 
  data = adastra_dat_pairs
)
...
adastra_dat_pairs <- adastra_dat_pairs %>%
  ...
  mutate(
    swab_hl = if_else(
      row_number() == order(-log10_viral_load, row_number())[1],
      "High",
      "Low"
    )
  ) %>%
  ungroup() %>%
  mutate(swab_hl = factor(swab_hl, levels = c("Low", "High")))

fit_hl <- lmer(
  log10_viral_load ~ swab_hl + Timepoint_ID + (1 | ID), 
  data = adastra_dat_pairs
)
...
```{r}
plot_name <- here("Plots", intervention, paste0("box_vl_fluType.png"))
png(plot_name, width = 8, height = 6, units = "in", res = 600)
print(box_vl_flutype)
dev.off()
```

```{r}
plot_name <- here("Plots", intervention, paste0("box_vl_fluType.pdf"))
pdf(plot_name, width = 8, height = 6)
print(box_vl_flutype)
dev.off()
```
...
```{r}
plot_name <- here("Plots", intervention, paste0("box_vl.png"))
png(plot_name, width = 8, height = 6, units = "in", res = 600)
print(box_vl)
dev.off()
```
...
```{r}
plot_name <- here("Plots", intervention, paste0("box_vl.pdf"))
pdf(plot_name, width = 8, height = 6)
print(box_vl)
dev.off()
```
...
if (use_threshold) {
  ...
  model_setup_f_name = paste0('Rout/model_run_setup_',intervention, '_vs_', ifelse(ref_arm == "No Study Drug", "NSD", ref_arm), '.RData')
}
save(model_settings, 
     adastra_dat_analysis,
     stan_inputs, 
...
tic()  # Start timing
if(RUN_MODELS){
  system(paste('Rscript --vanilla run_models_local.R',intervention, mITT_threshold, ifelse(ref_arm == "No Study Drug", "NSD", ref_arm), use_threshold))
}
...
if (use_threshold) {
  ...
  ff = list.files('Rout/', pattern = paste0(intervention, '_vs_', ifelse(ref_arm == "No Study Drug", "NSD", ref_arm), '.RData'))
}
...
```{r}
# save half life data
Half_life_dat <- Half_life %>% select(-Trt, -ID_stan)
Half_life_dat <- Half_life_dat %>%
  mutate(ID = as.character(ID)) %>%
  arrange(ID)

write.csv(
  Half_life_dat,
  "D:/baloxavir-favipiravir-oseltamivir-adastra/Analysis_Data/Half_life_dat_ITT.csv",
  row.names = FALSE
)
```
...
```{r}
plot_name <- here("Plots", intervention, paste0("hl_fluType.png"))
png(plot_name, width = 8, height = 6, units = "in", res = 600)
print(G_hl_flutype)
dev.off()
```
...
```{r}
plot_name <- here("Plots", intervention, paste0("hl_fluType.pdf"))
pdf(plot_name, width = 8, height = 6)
print(G_hl_flutype)
dev.off()
```
...
```{r}
if(ref_arm == "No Study Drug"){
  plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_esimated_effect_vs_nsd.png"))

png(plot_name, width = 14, height = 6, units = "in", res = 600)
grid.arrange(G_hl, G2, ncol = 2)
dev.off()
}
```
...
```{r}
if(ref_arm == "No Study Drug"){
  plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_esimated_effect_vs_nsd.pdf"))

pdf(plot_name, width = 14, height = 6)
grid.arrange(G_hl, G2, ncol = 2)
dev.off()
}
```
...
```{r}
if(ref_arm != "No Study Drug"){
  plot_name <- here("Plots", intervention, paste0("mITT_", mITT_threshold, "_esimated_effect_vs_positive_arm.pdf"))
