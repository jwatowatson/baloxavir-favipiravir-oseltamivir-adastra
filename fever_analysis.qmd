...
temp_dat <- merge(temp_dat, adastra_dat[,c("ID", "Trt", "Rand_date")],
                  by.x = "Label", by.y = "ID")
...
```{r}
fit <- survfit(Surv(clearance_time, clearance_time_cens) ~ Trt, data = temp_dat_for_plot) 
diff=survdiff(Surv(clearance_time, clearance_time_cens) ~ Trt, data = temp_dat_for_plot) 
...
for(i in 1:length(trts)){
  trt_i <- trts[i]
  temp <- temp_dat_for_plot2 %>% filter(Trt %in% c(ref_arm, trt_i))
