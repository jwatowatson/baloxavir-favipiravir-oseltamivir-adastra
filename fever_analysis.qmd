...
```{r}
# For Baseline characteristic table
temp_dat %>% filter(mITT) %>%
  distinct(Label, .keep_all = TRUE) %>%
  count(Trt, Fever_Baseline) %>%
  group_by(Trt) %>%
  mutate(
    pct = 100 * n / sum(n)
  )
...
table(summary_temp_values$Fever_Baseline, summary_temp_values$Trt)
...
summary_temp_values <- summary_temp_values %>%
  mutate(Trt = relevel(factor(Trt, levels = trt_order), ref = ref_arm))

cox_fit <- coxph(Surv(clearance_time, clearance_time_cens) ~ Trt, data = summary_temp_values)

hr_tab <- summary(cox_fit)$conf.int
hr_tab <- as.data.frame(hr_tab) %>%
...
pairwise_survdiff(Surv(clearance_time, clearance_time_cens) ~ Trt, data = summary_temp_values)
```
...
rmean_table %>%
  mutate(ci95_up = rmean + 1.96*`se(rmean)`,
         ...
         lab = paste0(round(rmean,2), " days [95%CI: ", round(ci95_low,2), " to ", round(ci95_up,2), " days]")) %>% select(strata, lab)
```

```{r}
# Compute absolute mean difference between treatment (in hours)
# Extract reference mean
ref_mean <- rmean_table %>%
  filter(strata == paste0("Trt=", ref_arm)) %>%
  pull(rmean)

# Compute mean reduction
mean_diff_table <- rmean_table %>%
  filter(strata != paste0("Trt=", ref_arm)) %>%
  mutate(
    mean_reduction_days = ref_mean - rmean,
    mean_reduction_hours = mean_reduction_days * 24
  )

mean_diff_table %>% select(strata, mean_reduction_days, mean_reduction_hours)
```
...
```{r fever_clearance}
survplot_fever <- survminer::ggsurvplot(fit,pval=F,
...
lab <- if (p_val < 1e-3) sprintf("p-value < 0.001") else sprintf("p-value = %.2f", p_val)

survplot_fever$plot <- survplot_fever$plot + 
...
survplot_fever$table <- survplot_fever$table +
                    labs(title = "Number at risk (number censored)") +
                    xlab(xlabel) +
...
print(survplot_fever, newpage = F)
dev.off()
```
...
```{r}
# From symptom clearance
grob2 <- arrangeGrob(survplot_sympt$plot, survplot_sympt$table, ncol = 1, heights = c(2, 1))
```

```{r}
fname <- paste0(here("Plots", intervention, "fever_symptom_surv_plot_28days.png"))

png(fname, width = 16, height = 8, unit = 'in', res = 600)
print(ggarrange(grob1, grob2, ncol = 2, nrow = 1))
dev.off()
```

```{r}
fname <- paste0(here("Plots", intervention, "fever_symptom_surv_plot_28days.pdf"))
