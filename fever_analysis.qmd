...
temp_dat <- merge(temp_dat, adastra_dat[,c("ID", "Trt", "Rand_date")], by.x = "Label", by.y = "ID")
...
temp_dat_for_plot$rangrp <- as.factor(temp_dat_for_plot$rangrp)
temp_dat_for_plot$rangrp <- factor(temp_dat_for_plot$rangrp, levels = c(ref_arm, trts))
```
...
cols <- trt_colors[c(ref_arm, trts)]
...
for(i in 1:length(trts)){
  trt_i <- trts[i]
  temp <- temp_dat_for_plot2 %>% filter(rangrp %in% c(ref_arm, trt_i))
  diff_i =survdiff(Surv(clearance_time, clearance_time_cens) ~ rangrp, data = temp) 
...
```{r}
fname <- paste0(here("Plots", intervention, "fever_symptom_surv_plot_7days.pdf"))
